...
def export_main(context, path, settings, operator):
    ...
    data = {'version': VERSION, 'precision': precision,
            'rotation': settings['userotation'], 'age': settings['useage'], 'frames': [], 'sprite_rotation': False}
...
class ParticlePlayerExporter(Operator, ExportHelper):
    ...
    firstframe: IntProperty(name="First frame", min=0,
                            subtype='UNSIGNED', default=0)
    lastframe: IntProperty(name="Last frame", min=0,
                           subtype='UNSIGNED', default=20)
    framestep: IntProperty(name="Frame step", min=1,
                           subtype='UNSIGNED', default=1)
...
def menu_func_export(self, context):
    self.layout.operator(ParticlePlayerExporter.bl_idname,
                         text="A-Frame Particle Player")
